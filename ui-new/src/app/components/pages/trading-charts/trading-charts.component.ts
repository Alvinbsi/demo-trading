...
import { widget, onready, ChartingLibraryWidgetOptions, LanguageCode }
from '../../../../trading-charts-lib/charting_library/charting_library.min';
...
export class TradingChartsComponent implements OnInit {
    ...
    constructor(private _lstore:StorageService, private _req:ApirequestService, private _urls:ApiurlsService) {
    ...
    //private _datafeedUrl = 'https://apollo.business-software.in/statistics';
    private _datafeedUrl = 'https://apollo.business-software.in/statistics';
    ...
    getPrimaryActDetail() {
      ...
      if(userId != null) {
        ...
        this._req.fetchApiData( this._urls.getPrimaryActUrl, data ).subscribe(
          (data:any) => {
            // console.log('data24234',data)
            let resSucc = data.data[0];
            let resErr = data.error;
            if(resSucc != '') {
              this.compSymbol = resSucc.symbol;
              setTimeout(() => {
                this.getTradingCharts();
              }, 0);
            }
          },
          error => {
